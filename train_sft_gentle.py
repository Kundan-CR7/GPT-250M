...
# ==========================================
class AlpacaGentleDataset(Dataset):
    def __init__(self, block_size=1024, num_samples=5000):
        # We take a high-quality subset first to stabilize the model
    ...
    def __getitem__(self, idx):
        ...
        # ✅ CRITICAL: Mask everything except the assistant's response
...
def main():
    ...
    # LOAD THE ORIGINAL 381K CHECKPOINT (Do not use the collapsed one!)
    ...
    if os.path.exists(checkpoint_path):
        ...
        print("✅ Original Base Model Loaded. Ready for Gentle SFT.")

    model.to(device)
    
    # Smaller batch size for better gradient resolution
    dataset = AlpacaGentleDataset(block_size=config.block_size)
    loader = DataLoader(dataset, batch_size=4, shuffle=True) 
    
    # ⚡ LOWER LR: 1e-5 to prevent weight collapse
    optimizer = torch.optim.AdamW(model.parameters(), lr=1e-5, weight_decay=0.1)
    
    model.train()
    print(f"🚀 Starting Gentle SFT on {device}...")

    for epoch in range(1): # Start with just 1 epoch to check health
        for step, (x, y) in enumerate(loader):
            ...
            with torch.amp.autocast(device_type='cuda', dtype=torch.float16):
                logits = model(x)
                # Ensure the loss ignores the -100 tokens
            ...
            # Save a "check" model halfway
            if step == 200:
                torch.save(model.state_dict(), "/kaggle/working/gpt250m_sft_check.pth")

    torch.save(model.state_dict(), "/kaggle/working/gpt250m_sft_gentle_final.pth")
    print("🎉 Gentle SFT Complete.")
