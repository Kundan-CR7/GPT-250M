...
def main():
    ...
    # Apply the custom learning rate
    ...
    # Track the absolute best average loss
    best_loss = float('inf')
    optimizer.zero_grad()
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        for batch_idx, batch in enumerate(train_loader): 
            ...
            # Add actual un-scaled loss to epoch tracker
            actual_loss = loss.item() * args.grad_accum_steps
            total_epoch_loss += actual_loss
            num_batches += 1

            if batch_idx % 10 == 0:
                print(f"Epoch {epoch} | Batch {batch_idx} | DPO Loss: {actual_loss:.4f}")

            # Timer Check (30-minute rolling backup)
        ...
        # ==========================================
        # END OF EPOCH "BEST MODEL" CHECK
        # ==========================================
        avg_epoch_loss = total_epoch_loss / num_batches
        print(f"\n[📊 EPOCH {epoch} SUMMARY] Average Loss: {avg_epoch_loss:.4f}")

        if avg_epoch_loss < best_loss:
            print(f"[🏆 NEW HIGH SCORE] Loss improved from {best_loss:.4f} to {avg_epoch_loss:.4f}!")
            best_loss = avg_epoch_loss
            
            best_model_path = os.path.join(args.checkpoint_dir, 'dpo_best_model.pth')
            save_checkpoint(epoch, batch_idx, instruct_model, optimizer, best_model_path)
            print(f"Saved best model to {best_model_path}\n")
        else:
            print(f"[📉 NO IMPROVEMENT] Loss {avg_epoch_loss:.4f} did not beat best loss {best_loss:.4f}\n")
